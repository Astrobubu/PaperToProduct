...
import { Domain, Paper } from "@/types";

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { paperIds, domain, searchQuery } = body as {
      paperIds: string[];
      domain: Domain;
      searchQuery: string;
    };
    ...
    // Run extraction on all papers in parallel
    const extractions = await extractFromPapers(papers, domain, searchQuery);

    // Cache extractions in paper_analyses table (using raw_analysis JSONB)
